...
#2024 Sergey Kolokolchikov. Additional function to calculate dp/p = dE/E.
def plot_bunch_momentum_spread_evol(RFStation, h5data, output_freq = 1,
                           dirname = 'fig', show_plot = False):
    """
    Plot of r.m.s. 4-sigma dE [eV] as a function of time.
    """
    ...
    ndata = int(time_step/output_freq)
    t = output_freq*np.arange(ndata)
    bl = np.array(h5data["/Beam/sigma_dE"][0:ndata], dtype = np.double)
    bl *= 4
    bl[time_step:] = np.nan
    ...
    ax.plot(t, bl, '.')
    ax.set_xlabel(r"No. turns [T$_0$]")
    ax.set_ylabel (r"Momentum Spread, $\Delta E_{4\sigma}$ r.m.s. [eV]")
    if time_step > 100000:
        ax.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
    ...
    if show_plot:
        ...
        fign = dirname +'/bunch_momentum_spread.png'
        plt.savefig(fign)
    plt.clf()
