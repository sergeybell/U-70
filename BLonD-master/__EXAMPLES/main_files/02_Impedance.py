...
os.makedirs(this_directory + '../output_files/Test_06_fig', exist_ok=True)
...
# Tracking details
N_t = 6225           # Number of turns to track
...
bunchmonitor = BunchMonitor(ring, rf, beam,
                            this_directory + '../output_files/Test_06_output_data', Profile=profile)

format_options = {'dirname': this_directory + '../output_files/Test_06_fig'}
plots = Plot(ring, rf, beam, dt_plt, N_t,
             -250e-9/harm*4, 1000e-9/harm*4, -100e6, 100e6, xunit='s', separatrix_plot=True,
             Profile=profile, h5file=this_directory + '../output_files/Test_06_output_data',
...
for i in range(N_t):
    ...
    if (i % dt_plt) == 0:
        plot_induced_voltage_vs_bin_centers(total_ind_volt_ZoN, figure_index=i,
                                            style='.', dirname=this_directory + '../output_files/Test_06_fig/1')
        plot_induced_voltage_vs_bin_centers(total_ind_volt_time, figure_index=i,
                                            style='.', dirname=this_directory + '../output_files/Test_06_fig/2')
    # Track
    for m in map_:
        m.track()
...
test_string += '{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\n'.format(
    np.mean(beam.dE), np.std(beam.dE), np.mean(beam.dt), np.std(beam.dt))
with open(this_directory + '../output_files/Test_06_test_data.txt', 'w') as f:
    f.write(test_string)
...
plt.savefig(this_directory + '../output_files/Test_06_fig/fig.png')
