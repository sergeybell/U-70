...
os.makedirs(this_directory + '../output_files/U-70', exist_ok=True)
...
E_inj = 7.0e9        # Injection Energy, [eV] before JUMP
...
#parabolic(ring, rf, beam, 1.2*1e3*1e-9/h/2.5, seed=1)
...
bunchmonitor = BunchMonitor(ring, rf, beam,
                            this_directory + '../output_files/U-70', Profile=profile)

format_options = {'dirname': this_directory + '../output_files/U-70'}
plots = Plot(ring, rf, beam, dt_plt, N_t,
             700e-9/h*4, 2000e-9/h*4, -600e6, 600e6, xunit='s', separatrix_plot=True,
             Profile=profile, h5file=this_directory + '../output_files/U-70',
...
for i in range(1, N_t + 1):
    ...
    if (i % dt_plt) == 0:
        ...
        print("   Four-times r.m.s. bunch momentum spread %.6e" % (4. * beam.sigma_dE / beam.energy))
    if (i % dt_plt) == 0:
        plot_induced_voltage_vs_bin_centers(total_ind_volt_ZoN, figure_index=i,
                                            style='.', dirname=this_directory + '../output_files/U-70/1')
    # Track
    for m in map_:
        m.track()
    ...
    #beam.losses_longitudinal_cut(1.60e-7, 1.95e-7)

# For testing purposes
test_string += '{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\n'.format(
    np.mean(beam.dE), np.std(beam.dE), np.mean(beam.dt), np.std(beam.dt))
with open(this_directory + '../output_files/U-70/U-70.txt', 'w') as f:
    f.write(test_string)

source = r"/Users/Bell/GIT_REPOS/Protvino/BLonD-master/__EXAMPLES/output_files/U-70.h5"
target = r"/Users/Bell/GIT_REPOS/Protvino/BLonD-master/__EXAMPLES/output_files/U-70/U-70.h5"

shutil.copyfile(source, target)

#PLOT ADDITIONAL VOLTAGE
#plt.figure()
...
#plt.savefig(this_directory + '../output_files/U-70/fig.png')
