...
os.makedirs(this_directory + '../output_files/02_Imp_harm_fig', exist_ok=True)

# Simulation parameters -------------------------------------------------------
# Bunch parameters
N_b = 1.0e12                 # Design proton Intensity in NICA proton 13 GeV
...
E_inj = 5.0e9              # Injection Energy, [eV] Test
...
# Tracking details
N_t = 17000           # Number of turns to track
dt_plt = 500        # Time steps between plots
...
bigaussian(ring, rf, beam, T_s*1e-9/4/harm/1.7/6, 50e5, reinsertion=True, seed=1)
...
bunchmonitor = BunchMonitor(ring, rf, beam,
                            this_directory + '../output_files/02_Imp_harm_output_data', Profile=profile)

format_options = {'dirname': this_directory + '../output_files/02_Imp_harm_fig'}
plots = Plot(ring, rf, beam, dt_plt, N_t,
             -250e-9/harm*4, 1000e-9/harm*4, -100e6, 100e6, xunit='s', separatrix_plot=True,
             Profile=profile, h5file=this_directory + '../output_files/02_Imp_harm_output_data',
...
for i in range(N_t):
    ...
    if (i % dt_plt) == 0:
        plot_induced_voltage_vs_bin_centers(total_ind_volt_ZoN, figure_index=i,
                                            style='.', dirname=this_directory + '../output_files/02_Imp_harm_fig/1')
#        plot_induced_voltage_vs_bin_centers(total_ind_volt_time, figure_index=i,
#                                            style='.', dirname=this_directory + '../output_files/02_Imp_harm_fig/2')
    # Track
    for m in map_:
        m.track()
...
test_string += '{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\n'.format(
    np.mean(beam.dE), np.std(beam.dE), np.mean(beam.dt), np.std(beam.dt))
with open(this_directory + '../output_files/02_Imp_harm_test_data.txt', 'w') as f:
    f.write(test_string)
...
plt.savefig(this_directory + '../output_files/02_Imp_harm_fig/fig.png')
