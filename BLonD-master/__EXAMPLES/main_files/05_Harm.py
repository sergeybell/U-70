...
os.makedirs(this_directory + '../output_files/05_Harm_fig', exist_ok=True)
...
bunchmonitor = BunchMonitor(ring, rf, beam,
                            this_directory + '../output_files/05_Harm_output_data', Profile=profile)

format_options = {'dirname': this_directory + '../output_files/05_Harm_fig'}
plots = Plot(ring, rf, beam, dt_plt, N_t,
             -250e-9/harm*4, 1000e-9/harm*4, -100e6, 100e6, xunit='s', separatrix_plot=True,
             Profile=profile, h5file=this_directory + '../output_files/05_Harm_output_data',
...
test_string += '{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\n'.format(
    np.mean(beam.dE), np.std(beam.dE), np.mean(beam.dt), np.std(beam.dt))
with open(this_directory + '../output_files/05_Harm_text_data.txt', 'w') as f:
    f.write(test_string)
